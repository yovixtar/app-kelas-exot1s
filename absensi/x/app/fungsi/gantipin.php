...
if (isset($_POST['gantipin'])) {
	$datapin=mysqli_fetch_array(mysqli_query($l,"SELECT * FROM login_absensi"));
	if (md5($_POST['pinlama']) == $datapin['pin']) {
		$a=mysqli_query($l,"INSERT INTO login_absensi SET pin=md5('".$_POST['pinbaru']."')");
		if ($a) {
			mysqli_query($l,"DELETE FROM login_absensi where pin=md5('".$_POST['pinlama']."') ");
		?>
